            ...
            dotint=NULL, panelcolor=NULL, tintmaxdiff=NULL) 
  {
    ...
    for(j in jSet)
      for(i in iSet) {
        ...
        if (!is.null(xlim) & is.null(ylim)) localPlot(x[, j], x[, i], xlab = "", ylab = "", axes = FALSE, type = "n", ..., log = l, xlim = c(0,3))
        if (!is.null(xlim) & !is.null(ylim)) localPlot(x[, j], x[, i], xlab = "", ylab = "", axes = FALSE, type = "n", ..., log = l, xlim = c(0,3), ylim=ylim[j,i,])
